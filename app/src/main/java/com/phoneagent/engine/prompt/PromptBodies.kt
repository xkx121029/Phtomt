...
/**
 * 提示词区块正文：逐字节搬迁自 [com.phoneagent.engine.AgentPrompts] 原有提示词字面量。
 *
 * ⚠️ 这里的每个字符串就是“AI 原样收到的提示词文字”，改动一个字符都会改变 AI 的行为。
 * 唯一允许的改写是把 Kotlin 插值换成 `{占位符}`，由 [PromptAssembler] 在装配时用 [PromptVars] 渲染；
 * 等价性由 PromptSnapshotTest 的金样本逐节锁定（指纹相等即逐字节相等）。
 *
 * 命名规则：`<区块 id 大写>_CN` / `_EN`，与 [PromptCatalog] 中的区块一一对应。
 */
internal object PromptBodies {
    ...
    User task: {task}
    User preferences (relevant only): {profile}
    ...
    // ---- plan.output_format ----
    internal val PLAN_OUTPUT_FORMAT_CN: String = """
    ...
    """.trimIndent()
    internal val PLAN_OUTPUT_FORMAT_EN: String = """
    ...
    // ---- dec.always.device_query ----
    internal val DEC_ALWAYS_DEVICE_QUERY_CN: String = """
    - 需要本机事实（应用清单、时间、电量、网络、存储）→ device_query（kind=apps/time/battery/network/storage/all），结果会作为上一步结果回给你；不要翻设置页，也不要每步都查。
    """.trimIndent()
    internal val DEC_ALWAYS_DEVICE_QUERY_EN: String = """
    ...
    // ---- rev.system ----
    internal val REV_SYSTEM_CN: String = """
    ...
    不要输出任何其它内容。
    """.trimIndent()
    internal val REV_SYSTEM_EN: String = """
    ...
    // ---- dist.memory ----
    internal val DIST_MEMORY_CN: String = """
    【记忆提炼】
    ...
    只输出 JSON。禁止 ```json 标记，禁止 JSON 前后任何文字。
    """.trimIndent()
    internal val DIST_MEMORY_EN: String = """
    【Memory Distillation】
